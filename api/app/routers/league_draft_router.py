...
from api.app.domain.commands.league.bid import (BidCommand, BidCommandExecutor,
                                                create_bid_command_executor)
from api.app.domain.commands.league.nominate_player import (
    NominatePlayerCommand, NominatePlayerCommandExecutor,
    create_nominate_player_command_executor)
from api.app.domain.commands.league.pass_bid import (
    PassBidCommand, PassBidCommandExecutor, create_pass_bid_command_executor)
from api.app.domain.commands.league.pause_resume_draft import (
    PauseResumeDraftCommand, PauseResumeDraftCommandExecutor,
    create_pause_resume_draft_command_executor)
from api.app.domain.commands.league.start_draft import (
    StartDraftCommand, StartDraftCommandExecutor,
    create_start_draft_command_executor)
from api.app.domain.commands.league.undo_last_draft_pick import (
...
async def start_draft(
    league_id: str,
    command_executor: StartDraftCommandExecutor = Depends(create_start_draft_command_executor)
):
...
async def nominate(
    ...
    command_executor: NominatePlayerCommandExecutor = Depends(create_nominate_player_command_executor)
):
    return command_executor.execute(command)
...
async def bid(
    command: BidCommand,
    command_executor: BidCommandExecutor = Depends(create_bid_command_executor)
):
    return command_executor.execute(command)
...
@router.post("/pass")
async def pass_bid(
    command: PassBidCommand,
    command_executor: PassBidCommandExecutor = Depends(create_pass_bid_command_executor)
):
    return command_executor.execute(command)
...
async def pause_draft(
    league_id: str,
    command_executor: PauseResumeDraftCommandExecutor = Depends(create_pause_resume_draft_command_executor)
):
...
async def undo_last_pick(
    league_id: str,
    command_executor: UndoLastDraftPickCommandExecutor = Depends(create_undo_last_draft_pick_command_executor)
):
...
async def select_player(
    ...
    command_executor: SelectPlayerCommandExecutor = Depends(create_select_player_command_executor)
):
    return command_executor.execute(command)
