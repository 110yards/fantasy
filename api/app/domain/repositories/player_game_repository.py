...
class PlayerGameRepository():
    ...
    def set(self, season: int, player_game: PlayerGame, transaction: Transaction = None):
        return self.firestore.set(PlayerGameRepository.path(season, player_game.player_id), player_game, transaction)

    def where(self, season: int, player_id: str, queries: Union[Query, List[Query]], transaction: Transaction = None) -> List[PlayerGame]:
