<template>
        ...
        <template>
          ...
          <tbody v-if="playersToApprove && playersToApprove.length != 0">
            <tr v-for="player in playersToApprove" :key="player.id">
              ...
              <td>
                <span class="team-icon" :class="player.team.abbr">{{ player.team.abbr }}</span>
              </td>
  ...
  <v-card v-else>
    ...
    <v-card-text>
      {{ reviewPlayer.display_name }} - {{ reviewPlayer.team.abbr.toUpperCase() }} <br />
    ...
    <v-card-text v-if="matchPlayer">
      ...
      <div class="mb-5">
        ...
        <label>Name:</label> {{ matchPlayer.display_name }} <br />
        <label>Date of birth: </label>{{ formatDate(matchPlayer.birth_date) }}
      </div>
      <div class="mb-5">
        <h3>Updated player</h3>
        <label>Name:</label> {{ reviewPlayer.display_name }} <br />
        <label>Date of birth: </label>{{ formatDate(reviewPlayer.birth_date) }}
      </div>
        ...
        <template>
          ...
          <tbody v-if="anyMatches">
            <tr v-for="player in matches" :key="player.id">
              <td>{{ player.display_name }}</td>
              <td>
                <span v-if="!player.is_free_agent" class="team-icon" :class="player.team.abbr">{{
  ...
  computed: {
    ...
    matches() {
      ...
      for (let player of this.players) {
        ...
        let matchA = player.display_name.toLowerCase().includes(this.searchCriteria.toLowerCase())
