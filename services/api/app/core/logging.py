...
class Logger:
    ...
    @classmethod
    def initialize(cls, is_dev: bool, gcloud_project: Optional[str], service_name: Optional[str], region: Optional[str]):
        cls._instance = cls.__new__(cls)

        if not is_dev:
            cls._logger = StackDriverLogger(gcloud_project, service_name, region)
        else:
            cls._logger = DefaultLogger()
