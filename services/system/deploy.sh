...
usage_tip="usage: deploy.sh 'tag' 'registry' 'target_project' 'endpoint'"
...
gcloud builds submit --config cloudbuild.deploy.yaml --substitutions=_TAG="$tag",_REGISTRY="$registry",_TARGET_PROJECT_ID="$target_project",_ENDPOINT="$endpoint"