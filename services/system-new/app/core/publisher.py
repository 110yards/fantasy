...
from app.yards_py.domain.repositories.virtual_pubsub_repository import VirtualPubSubPayload, VirtualPubsubRepository
    ...
    def create_topic(self, topic_name: str):
        ...
        try:
            try:
                ...
                StriveLogger.info("Topic exists", extra={"topic_name": topic_name})
            except NotFound:
                self.publisher.create_topic(name=topic_path)
                StriveLogger.info("Topic created", extra={"topic_name": topic_name})
        except BaseException as ex:
            StriveLogger.error("Failed to create topic", extra={"topic_name": topic_name}, exc_info=ex)
            raise ex
        ...
        with subscriber:
            try:
                try:
                    ...
                    StriveLogger.info("Push subscription exists", logging_extra)
                    return subscription
                except NotFound:
                    ...
                    StriveLogger.info("Push subscription created", logging_extra)
                    return subscription

            except BaseException as ex:
                StriveLogger.error("Failed to create push subscription", exc_info=ex, extra=logging_extra)
                raise ex
...
class VirtualPubSubPublisher(Publisher):
    ...
    def create_topic(self, topic_name: str):
        ...
        StriveLogger.info(f"Created virtual topic '{topic_name}'")
    ...
    def create_push_subscription(self, subscription_name: str, topic_name: str, endpoint: str, config: SubscriptionConfig = None):
        ...
        StriveLogger.info(f"Created virtual push subscription '{subscription_name}' on topic '{topic_name}' to {endpoint}")
        return Subscription(name=subscription_name)


def get_message_data(push: Dict) -> Dict:
    StriveLogger.info(f"[PUB/SUB] {push}")
