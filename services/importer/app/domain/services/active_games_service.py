...
from app.domain.constants import tsn
...
class ActiveGamesService:
    ...
    def get_games(self, hours: int) -> list[Game]:
        ...
        # tightly coupled to TSN
        self.players = {player.tsn_id: player for player in players.values()}
        ...
        in_scope = [
            game for game in schedule.games.values() if scope_start <= parser.isoparse(game.date_start) <= scope_end
        ]
        ...
        game.rosters = Rosters(
            teams=RosterTeams(
                team_1=RosterTeam(
                    ...
                    roster=[
                        self.to_roster_player(p)
                        for p in self.rostered_players.values()
                        if p.team.team_id == schedule_game.team_1.team_id
                    ],
                ),
                team_2=RosterTeam(
                    ...
                    roster=[
                        self.to_roster_player(p)
                        for p in self.rostered_players.values()
                        if p.team.team_id == schedule_game.team_2.team_id
                    ],
                ),
            )
        )
